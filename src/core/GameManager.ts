...
export class GameManager {
  ...
  public addPlayer(ws: WebSocket) {
    ...
    ws.send(
      JSON.stringify({
        message: `Player ${++GameManager.playersCount} connected, give your name : `,
      })
    );
  ...
  public removePlayer(ws: WebSocket) {
    GameManager.players = GameManager.players.filter(
      (player) => player.ws !== ws
    );
    //stop his game
    ...
    if (gameIndex !== -1) {
      ...
      if (game.whitePlayer.ws === ws) {
        game.blackPlayer.ws.send(
          JSON.stringify({ type: "error", message: "Opponent left the game." })
        );
      } else {
        game.whitePlayer.ws.send(
          JSON.stringify({ type: "error", message: "Opponent left the game." })
        );
      }
    ...
    player.ws.on("message", (message) => {
      ...
      if (messageData.type === Set_Name) {
        ...
        GameManager.players.forEach((p) => {
          if (p.ws !== player.ws) {
            p.ws.send(
              JSON.stringify({ type: Player_Joined, payload: player.name })
            );
          }
        });
      }
